...
**NOTE:** The default locations of the `BatchValidationSettings` resource is `res://test/batch_validation_settings.tres` and that's the default path used by the Godot Doctor settings. However, that file is not supplied, it needs be create manually.  
